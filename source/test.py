...
class TestGraphBuilder(unittest.TestCase):
    ...
    def test_set_starting_state_in_initial_mode(self):
        with self.assertRaisesRegex(om.OMError, "Can't run function set_starting_state in State mode."):
            _ = (
                om.GraphBuilder("TEST")
                .State("A", model=om.ConstModel("to_a"))#.Action("to_a", next_state="A")
                .set_starting_state("A")
                .Build()
            )

    def test_starting_state_valid(self):
        with self.assertRaisesRegex(om.OMError, "Starting state non-state is not in states: \[\]"):
    ...
    def test_state_model_is_string_or_model(self):
        with self.assertRaisesRegex(om.OMError, "Model for State A must be a string or a Model"):
            _ = (
                om.GraphBuilder("TEST")
                .State("A", model=1)
                .set_starting_state("A")
    ...
    def test_state_model_cannot_be_udm(self):
        ...
        with self.assertRaisesRegex(om.OMError, "UDM my_udm must be registered and referred to by name"):
            _ = (
                om.GraphBuilder("TEST")
